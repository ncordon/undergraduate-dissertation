...
Se puede probar que el espacio probabilístico producto sólo hay uno, pero no usaremos este hecho. Nótese que cuando 
tengamos un espacio de probabilidad $(\Sigma, X)$ y consideramos que podemos extraer $m$ muestras del mismo
de manera independiente, estaremos sólo midiendo conjunto $A=A_1 \times A_2 \times \ldots \times A_m$, con $A_i \in\Sigma_i$.
Para estos sucesos siempre tendremos $\overline{P}(A) = \prod_{i=1}^m P(A_i)$ sea cual sea la distribución producto
$(\underset{1)}{\Sigma} \otimes \ldots \otimes \underset{m)}{\Sigma}, \overline{P})$, pero conviene ser consciente de que
de que es única.
