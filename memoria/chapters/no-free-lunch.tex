...
El equilibrio error-varianza (en inglés, \emph{bias-variance tradeoff}) hace referencia a un concepto intuitivo que se 
desprende de la demostración del teorema \ref{th:nfl}. La clave de dicha demostración ha sido que estabamos considerando
como conjunto de clasificadores todo el espacio $2^X$. Así, intuitivamente, cuanto más complejo o grande sea el conjunto 
$H$, mejor aproximaremos el verdadero o mejor clasificador, pero más número de muestras necesitaremos y más varianza 
respecto al mejor clasificador tendremos.

\begin{definition}
 Llamamos error de aproximación a $err_{app} := \inf_{h\in H} L_{\dist}(h)$. LLamamos error de estimación a $err_{est,S} := 
