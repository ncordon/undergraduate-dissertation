...
Puesto que aproximar cualquier distribución cuando se verifica la hipótesis de factibilidad es una condición mucho menos 
fuerte que poder aproximar el mejor algoritmo de la clase de funciones, aunque pueda parecer contraintuitivo, la clases
APAC cognoscibles están contenidas en las PAC cognoscibles. De hecho, tenemos una relación tal cual se muestra en la figura
siguiente:

\img{./imgs/clases-pac.png}{0.75}

\begin{fact}
 Los conceptos de PAC cognoscibilidad para una clase $H$ que verfica hipótesis de factibilidad y de APAC cognoscibilidad 
 con funciones de pérdida $0-1$ son equivalentes
\end{fact}

\begin{proof}
 Basta comprobar que coincidirían ambos conceptos de error y que como se verifica la hipótesis de 
 factibilidad, $\inf_{h\in H} L_{\dist}(h) = 0$.
\end{proof}
